...
def extraction_examples():
    example_1 = """
Hint: Please answer the question with Yes or No at the end.\n
Question: Is the cat black?\n
Model response: After checking the image, the answer is Yes.\n
Extracted answer: <answer>Yes</answer>
"""

    example_2 = """
Hint: Please answer the question with Yes or No at the end.\n
Question: Is the stop sign blue?\n
Model response: No, the stop sign is not blue.\n
Extracted answer: <answer>No</answer>
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="HallusionBench API Judge ArgParser")
...
def construct_messages_for_judge(dataitem: dict):
    ...
    gt_answer = "Yes" if str(dataitem["gt_answer"]) == "1" else "No"
...
def read_hallusionbench_eval_data(filepaths: list[str]):
    all_data = []
    for filepath in tqdm.tqdm(filepaths, desc="Loading Data files"):
        try:
            ...
            for item in items:
                data_id = item.get("question_id", "unknown")
                all_data.append({
...
def main(args):
    ...
    data_for_judge = read_hallusionbench_eval_data(filepaths=filepaths)
