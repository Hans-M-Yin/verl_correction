...
Model response: The car is blue, so the answer is B.\n
Extracted answer: <answer>B</answer>
"""

    example_2 = """
Hint: Please answer the question briefly and provide the final phrase at the end.\n
Question: What word is written on the sign?\n
Model response: The sign reads STOP.\n
Extracted answer: <answer>STOP</answer>
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="FREAK API Judge ArgParser")
...
def construct_messages_for_judge(dataitem: dict):
    ...
    if dataitem.get("options") is None:
        user_query = openend_template.format(
            question=dataitem["question"],
            response=response,
            gt_answer=dataitem["ground_truth"],
        )
    else:
...
def read_freak_eval_data(filepaths: list[str]):
    all_data = []
...
def main(args):
    ...
    data_for_judge = read_freak_eval_data(filepaths=filepaths)
